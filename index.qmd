...
```{r get data}
dat <- jsonlite::fromJSON("https://maps2.bristol.gov.uk/server2/rest/services/ext/ward_profiles/MapServer/17/query?f=json&where=(YEAR%20IN%20('2020'))&outFields=*", flatten = TRUE)
```
...
dat %>% glimpse()                           # provides a glimpse of the output
```
...
```{r analysis}
dat %>% mutate(total = rowSums(across(c(5:22))),           # total jobs in a ward
...
```{r}
oa.ew <- st_read("https://maps2.bristol.gov.uk/server2/rest/services/ext/ll_boundaries/MapServer/4/query?outFields=*&where=1%3D1&f=geojson")
```

Then, we need to merge the `oa.ew` spatial dataframe that contains the spatial information for Bristol's wards
`dat` dataframe that contains all the job data. We can do this with the `left_join()` function from the `dplyr` package and then create a map.

```{r}
left_join(oa.ew, dat, by = c("WARD_ID" = "WARD_CODE")) %>% 
